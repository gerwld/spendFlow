...
import { accountsSelectors, appSelectors, categoriesSelectors } from '@redux'
...
const EditAccountsScreen = ({ navigation }) => {
  const dispatch = useDispatch();
  const [themeColors] = useCurrentTheme();
  const isInit = useSelector(appSelectors.isAccountsInit)
  ...
  useFocusEffect(() => {
    ...
    itemsIDs = accountsSelectors.selectAccountsArray(state);
    setItems(itemsIDs)
  })
  ...
  const onSort = (IDsArray) => {
    if(Array.isArray(IDsArray) && IDsArray.length === items.length)
      dispatch(accountsActions.swapAccountsIDs(IDsArray))
  }

  if(!isInit || !items) return null;
  return (
    <BaseView>
      <STHeader {...{ navigation, title: "Manage accounts" }} />
...
const RenderAccountItem = ({ itemID }) => {
  ...
  const item = useSelector((state) => accountsSelectors.selectAccountByID(state, itemID))

  const onEditPress = () => {
    navigateWithState("editaccount")
  }
...
export default EditAccountsScreen